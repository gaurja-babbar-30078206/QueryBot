...
# from home_page_view_model import initialise_embed_llm ,init_embed_llm_llama,initialise_llm
...
with st.sidebar:
    ...
    if st.radio:
        ...
        lang_flow = LangchainImplementation()
        llm = lang_flow.initialise_llm(left_col.selectbox(
        "LLM",placeholder = "Choose an LLM",
        options= range(len(llm_path_list)), index= None,
        format_func= lambda x: llm_path_list[x].model_file,
        ))

    embed_llm = lang_flow.initialise_embed_llm(right_col.selectbox(
    ...
    if embed_llm is not None:
        embeddings = embed_llm.get_text_embedding("Hello World!")
        blog(f"Length of Embeddings ----> {len(embeddings)}")
